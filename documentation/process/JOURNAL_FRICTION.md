...
| F010 | 2026-07-27 | Le garde CI « Isolation des schémas Postgres » (job `architecture`) rend des **faux positifs**, et il en rendait déjà sur `main` | Son motif `\b[a-z][a-z0-9_]*\.[a-z][a-z0-9_]*` s'applique au fichier ENTIER, commentaires compris. Il accuse donc `verify.sql`, `provision.sql`, `securite.md`, `user.registered` — de la prose — d'être des schémas étrangers, et jusqu'aux séquences `\n` `\r` des chaînes SQL. Vérifié en rejouant le garde sur le contenu de `main` : **9 faux positifs**. Le job `architecture` ne peut donc pas être vert, et ce garde ne garde rien | **RÉSOLU** le 2026-07-27 — le garde vit désormais dans `tools/verifie-isolation-schemas.sh`, **une seule définition** appelée par `ci.yml` et par `task ci:schemas`. Il ne compare que le SQL **exécutable** : commentaires `--` et chaînes littérales retirés avant analyse. Corrigé aussi : la boucle traitait le nom du dossier comme un MODULE alors que `migrations/{moteur}/` porte un MOTEUR. Livré avec son **cas d'échec versionné** (ADR 013), `tools/testdata/isolation-violation/` — un script rendu muet fait échouer le garde, c'est vérifié |

| F011 | 2026-07-27 | **CodeQL ne peut pas s'exécuter** : « Code scanning is not enabled for this repository ». Le dépôt est repassé privé le 2026-07-27, et l'analyse de code n'est pas disponible pour un dépôt privé sur ce plan | Le workflow `codeql.yml` échoue à l'étape `analyze`. Ce n'est **pas un défaut du code** : `autobuild` réussit, c'est la REMONTÉE des résultats qui est refusée. Conséquence directe : **aucune analyse statique de sécurité ne tourne**, et le contrôle reste rouge sur toute PR visant `main`. Même famille que F002 — deux gardes que le dépôt a écrits et que le compte ne peut pas exécuter | **Ouvert, ASSUMÉ** — arbitrage issue #72, décision prise le 2026-07-27 : **le job n'est ni désactivé ni rendu non bloquant**. Un job vert qui n'analyse rien est exactement la forme que l'ADR 013 combat, et le dépôt vient d'en extirper onze. **Un contrôle rouge qui signifie « indisponible » vaut mieux qu'un contrôle vert qui ne signifie rien.** Fusionner vers `main` est donc, tant que ceci dure, une décision **documentée** — jamais un gate franchi. Remèdes, tous hors ingénierie : repasser le dépôt public (écarté — #61, aucun fichier `LICENSE`), un plan payant (règlerait aussi F002), ou assumer. `govulncheck` n'est pas concerné : il tourne et rend 0 vulnérabilité |
